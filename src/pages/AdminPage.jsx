...
// Importamos o hook de autenticação, presumindo que ele existe
...
const navItems = [
   { key: "visao-geral", name: "visão-geral.jsx", icon: FaRegChartBar },
...
const SidebarIcons = ({ selectedIcon, setSelectedIcon }) => (
 ...
 ${selectedIcon === item.key ? "drop-shadow-[0_0_1px_currentColor]" : ""}`}
            />
         </button>
      ))}
   </div>
...
const SidebarNav = ({ user, selectedNav, setSelectedNav }) => {
   // Novo estado para controlar a visibilidade da lista de navegação
   const [isFolderOpen, setIsFolderOpen] = useState(true); // Função para alternar o estado da pasta
   ...
   return (
      <div className="flex flex-col bg-[#21222c] border-r border-[#44475a] min-w-[250px]">
         ...
         {/* Pastas e arquivos */}
         {/* Adicionamos o evento onClick e um cursor de ponteiro */}
         <div
            className="flex items-center gap-1.5 pl-3 text-white cursor-pointer font-medium"
            onClick={toggleFolder}
         >
            {/* A seta agora gira com base no estado da pasta */}
            <span
         ...
         {/* Lista de navegação dinâmica - Renderizada apenas se a pasta estiver aberta */}
         {isFolderOpen && (
            <div className="w-full mt-2 text-sm space-y-1 font-medium">
               {navItems.map((item) => (
                  <button
                     ...
                     onClick={() => setSelectedNav(item.key)} // Classes de espaçamento corrigidas
                     className={`flex items-center w-full gap-2 p-1 pl-13 pr-3 transition-colors duration-200
  ${
...
const MainContent = ({ user, selectedNav, setSelectedNav }) => {
   // Encontra o item de navegação selecionado
         ...
         case "visao-geral":
            return (
               <p className="mt-4 text-[#6272a4]">
                  Este é o painel de visão geral.
               </p>
            );
         case "produtos":
            return (
   ...
   return (
      <div className="flex flex-col flex-grow bg-[#191a21] rounded-r-md ">
         <div className=" bg-[#282a36] w-full">
            <div className="flex border-b border-[#44475a] bg-[#21222c]">
               {/* Tab dinâmico para o item selecionado */}
               {navItems.map((item) => (
                  <button
                     key={item.key}
                     onClick={() => setSelectedNav(item.key)}
                     className={`border-r border-[#44475a] pr-12 p-2 pl-2 flex items-center gap-2 text-sm
  ${
     selectedNav === item.key
        ? "bg-[#282a36]"
        : "bg-[#21222c] hover:bg-[#44475a] text-[#6272a4]"
                  ...
                  >
                     ...
                     {item.name}
                     {item.key === "clientes" && (
                        <span className="rounded-full w-2 h-2 bg-amber-200 ml-2"></span>
                     )}{" "}
                  </button>
               ))}
            </div>
            <div className="w-full border-b border-[#44475a] py-2 gap-3 flex text-xs">
               <span className="text-[#6272a4] ml-6 s">dashboard </span>
...
export default function AdminPage() {
   ...
   return (
      <div className="flex w-screen h-screen font-sans text-white bg-gray-950 ">
         <div className="flex m-3 rounded-md flex-grow">
            <SidebarIcons
